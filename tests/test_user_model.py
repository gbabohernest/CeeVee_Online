...
class TestUserModel(unittest.TestCase):
    ...
    def test_count_category(self):
        ...
        count = count_by_id(category.id)  # Count the number of categories with the test category's ID
    ...
    def test_category_by_alias(self):
        ...
        assert retrieved_category_by_alias == category  # Check if the retrieved category matches the created category
    ...
    def test_user_model(self):
        ...
        user = User(first_name="Francis", last_name="George",
                    email="sangary7683@yahoo.com", password=hashed_pass, role="Assistant")
        app.app_context().push()
        db.session.add(user)
        db.session.commit()
